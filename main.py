import os, re, requests, feedparser, json, hashlib
from datetime import datetime, timezone, timedelta, date

# feedparser บาง feed ต้องมี User-Agent ไม่งั้นโดนบล็อก
UA = "Mozilla/5.0 (compatible; AiNewsBot/1.0; +https://telegram.org)"

def clean_html(raw):
    """ตัดแท็ก HTML และช่องว่างเกิน เหลือข้อความล้วน"""
    if not raw:
        return ""
    txt = re.sub(r"<[^>]+>", " ", raw)          # ตัดแท็ก
    txt = re.sub(r"&[a-zA-Z#0-9]+;", " ", txt)  # ตัด &nbsp; &amp; ฯลฯ
...
def fetch(urls, n=6, max_chars=600):
    """ดึงข่าวใหม่ พร้อมเนื้อข่าว (summary) ไม่ใช่แค่หัวข้อ"""
    out = []
    for url in urls:
        try:
            ...
            for e in feed.entries[:n]:
                ...
                # ดึงเนื้อข่าวจากหลายฟิลด์ที่ feed อาจใช้
                summary = ""
                ...
                # ตัดไม่ให้ยาวเกิน (กันเปลือง token) แต่ตัดที่ท้ายประโยค
...
def fmt(items):
    if not items:
        return "(ไม่มีข่าวใหม่)"
    lines = []
    for it in items:
        lines.append(f"- หัวข้อ: {it['title']}")
        if it.get("summary"):
            lines.append(f"  เนื้อหา: {it['summary']}")
    return "\n".join(lines)
...
2. ข่าวหุ้น
- เลือกเฉพาะข่าวที่มีผลต่อราคาหุ้น (เลือกมาให้ครบทุกข่าวที่สำคัญ ไม่ตัดทิ้ง)
- แต่ละข่าวเขียน 2-4 ประโยค ครอบคลุม 3 ส่วนนี้จากข้อมูลในข่าวจริง:
  (1) เกิดอะไรขึ้น — สรุปเหตุการณ์จากเนื้อข่าว ระบุรายละเอียดที่มี เช่น ตัวเลข ดีล บุคคล เหตุการณ์
  (2) ทำไมถึงกระทบ — อธิบายกลไกว่าเหตุการณ์นี้เชื่อมโยงกับผลประกอบการ/ต้นทุน/รายได้/ความเชื่อมั่นอย่างไร
  (3) ทิศทางที่คาด — มีแนวโน้มบวกหรือลบต่อหุ้นตัวนั้น (ระบุเฉพาะเมื่อมีเหตุผลรองรับในข่าว ถ้าไม่ชัดให้บอกว่า "ทิศทางยังไม่ชัด")
- ห้ามเขียนลอยๆ ว่า "มีข่าวที่อาจส่งผลต่อราคาหุ้น" โดยไม่บอกว่าข่าวคืออะไรและกระทบอย่างไร — ถ้าเนื้อข่าวไม่มีรายละเอียดพอ ให้ข้ามข่าวนั้นไป
...
- แต่ละข่าวเขียน 2-3 ประโยค: สรุปเหตุการณ์จากเนื้อข่าว + อธิบายว่าเชื่อมโยงกับตลาดการเงิน/สินทรัพย์ใด (หุ้น พันธบัตร น้ำมัน ทอง ค่าเงิน) อย่างไร
- ห้ามเขียนลอยๆ ว่า "อาจส่งผลต่อตลาดการเงิน" โดยไม่ระบุช่องทางที่กระทบ
...
เป้าหมาย: รายงานที่ถูกต้อง มีสาระ ลงรายละเอียดได้เท่าที่ข่าวมี ไม่สรุปเกินหลักฐานในข่าว
หมายเหตุ: ด้านล่างมีทั้ง "หัวข้อ" และ "เนื้อหา" ของแต่ละข่าว ให้ใช้เนื้อหาในการวิเคราะห์ อย่าดูแค่หัวข้อ
...
def split_message(msg, limit=4000):
    """แบ่งข้อความยาวโดยพยายามตัดที่ท้ายบรรทัด ไม่ตัดกลางประโยค"""
    parts, cur = [], ""
    for line in msg.split("\n"):
        # ถ้าบรรทัดเดียวยาวเกินลิมิต (เกิดยาก) ตัดแบบดิบ
